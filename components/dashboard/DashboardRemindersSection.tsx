...
function cannedReminder(firstName: string, service: string, when: string) {
  return `Hi ${firstName}, reminder: your ${service} is coming up at ${when}. Reply if you need to reschedule.`;
}
...
export default async function DashboardRemindersSection({
  ...
  // Suggestions are pre-generated by the reminders cron. No LLM on render.
  const reminders = reminderRows.map((appt) => {
    const when = formatDateTime(appt.scheduled_at);
    const reminderText =
      appt.suggested_reminder_message ??
      cannedReminder(
        appt.client_name.split(" ")[0] ?? "there",
        appt.service,
        when
      );
    return { ...appt, reminderText };
  });
