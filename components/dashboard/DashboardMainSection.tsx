...
export default async function DashboardMainSection({
  ...
  const {
    ...
    activity,
    errors,
  } = await getDashboardSnapshot(businessId);
    ...
    <>
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <MetricCard
          ...
          sub={`${completedMtdCount} completed appointments`}
        />
        <MetricCard
              ...
              {todayAppointments.map((appt) => (
                <div key={appt.id} className="rounded-xl border border-[#1E1E2A] bg-[#0E0E14] p-4">
                  ...
                  <div className="mt-2 flex items-center justify-between">
                    <p className="text-xs text-zinc-500">{formatDateTime(appt.scheduled_at)}</p>
              ...
              {activity.map((item) => (
                <div key={item.id} className="flex items-start gap-3 rounded-xl border border-[#1E1E2A] bg-[#0E0E14] p-4">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs text-zinc-500">{formatDateTime(item.sent_at)}</p>
                  </div>
                </div>
              ))}
            </div>
