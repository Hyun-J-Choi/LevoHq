...
export default function ImportCSVModal({
  ...
  function parseCSV(text: string): { parsed: ParsedRow[]; errors: string[] } {
    ...
    if (lines.length < 2) return { parsed: [], errors: ["CSV must have a header row and at least one data row."] };
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      parsed.push({ name, phone, service, last_visit_date });
    }
  ...
  async function handleImport() {
    ...
    // Batch insert — first upsert clients, then insert appointments
    for (const row of rows) {
      // 1. Upsert client (find by phone + business_id, or create)
      ...
      if (!clientId) {
        ...
        if (clientErr || !newClient) {
          importErrors.push(`Failed to create client "${row.name}": ${clientErr?.message ?? "unknown error"}`);
          continue;
        }
        clientId = newClient.id;
      } else {
        // Update last_visit if the imported date is more recent
        if (row.last_visit_date) {
          await supabase
            .from("clients")
            .update({ last_visit: new Date(row.last_visit_date).toISOString() })
            .eq("id", clientId);
        }
      }

      // 2. Insert completed appointment
      const { error: apptErr } = await supabase
        ...
        .insert({
          ...
          service: row.service,
          scheduled_at: row.last_visit_date ? new Date(row.last_visit_date).toISOString() : new Date().toISOString(),
          status: "completed",
        });

      if (apptErr) {
        importErrors.push(`Failed to create appointment for "${row.name}": ${apptErr.message}`);
        continue;
      }

      // 3. Grant SMS consent (import source) — skip if already exists
      ...
      if (consentResult.error && !consentResult.error.code?.includes("23505")) {
        console.warn("Consent insert warning:", consentResult.error.message);
      }
      ...
      <div className="w-full max-w-2xl rounded-2xl border border-white/[0.08] bg-[#111116] p-6 shadow-2xl space-y-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-neutral-500">Upload a CSV to import existing clients and their last visit.</p>
          </div>
          <button onClick={onClose} className="text-neutral-500 hover:text-white transition text-xl leading-none">&times;</button>
        </div>

        {/* Format guide */}
        <div className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-4">
          <p className="text-xs uppercase tracking-wider text-neutral-500 mb-2">Required CSV format</p>
          <code className="block text-sm text-[#D4A853] font-mono">
            name,phone,service,last_visit_date
          </code>
          <code className="block text-sm text-neutral-400 font-mono mt-1">
            Jane Smith,+15551234567,Botox,2025-12-15
          </code>
        </div>
